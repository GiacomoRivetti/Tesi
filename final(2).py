...
class Cluster:
    def __init__(self, nodes, base_graph=None):
        self.nodes = nodes
    ...
    def sample_outgoing_edge(self, base_graph, cluster_map, max_attempts=10):
        ...
        for node in self.nodes:
            for neighbor in base_graph[node.id]:
                if self.is_external_neighbor(neighbor, cluster_map):
        ...
        for _ in range(max_attempts):
            ...
            if self.is_external_neighbor(v, cluster_map):
                return (u, v)

    def merge(self, other):
        merged_nodes = list(set(self.nodes + other.nodes))
        return Cluster(merged_nodes, base_graph=self.base_graph)

        
    def is_external_neighbor(self, neighbor_id, cluster_map):
        return cluster_map.get(neighbor_id, self.min_id) != self.min_id

    def create_expander(self, log_degree=8, walk_length=5, cluster_map=None):
        ...
        for node in self.nodes:
            for neigh in node.neighbors:
                if cluster_map and self.is_external_neighbor(neigh, cluster_map):
                    external_edges.add(tuple(sorted((node.id, neigh))))

        tokens = []
        for node in self.nodes:
            for _ in range(log_degree // 2):
                tokens.append({"origin": node.id, "current": node.id})
        ...
        for node in self.nodes:
            node.new_neighbors = set(node.neighbors)
        ...
        for node in self.nodes:
            node.new_neighbors.update(v for v in self.base_graph[node.id] if v not in self.ids)
            node.neighbors.update(node.new_neighbors)
            del node.new_neighbors


    def degree_reduction(self, delta=8, walk_length=5, c=None, cluster_map=None):
        ...
        if cluster_map is not None:
            for node in self.nodes:
                external_edges[node.id] = {v for v in node.neighbors if cluster_map.get(v, self.min_id) != self.min_id}
        else:
            for node in self.nodes:
                external_edges[node.id] = set()


        for node in self.nodes:
            node.new_neighbors = set(v for v in node.neighbors if v in cluster_nodes)
        ...
        for token in tokens:
            arrivals[token["current"]].append(token)

        accepted_tokens = []
        for node_id, toks in arrivals.items():
            accepted = random.sample(toks, min(len(toks), delta))
        ...
        for node in self.nodes:
            node.new_neighbors.update(external_edges.get(node.id, set()))
            node.neighbors.update(node.new_neighbors)
            del node.new_neighbors
...
class OverlayNetwork:
    def __init__(self, graph=None, num_nodes=None):
        ...
        self.clusters = [Cluster([n], base_graph=self.base_graph) for n in self.nodes]
        ...
        with open('results.csv', 'w', newline='') as csvfile:
            ...
            for stage in range(stages):
                ...
                for c in self.clusters:
                    c.create_expander(log_degree=8, walk_length=10)
                    c.degree_reduction(delta=16, walk_length=10, cluster_map=cluster_map)
